...
func applyDatabaseURL() error {
	raw := strings.TrimSpace(os.Getenv("DOCS_DATABASE_URL"))
	if raw == "" {
		return nil
	}
	...
	if err != nil || parsed.Hostname() == "" || parsed.User == nil ||
		...
		return fmt.Errorf("DOCS_DATABASE_URL must be an absolute PostgreSQL URL")
	}
	name := strings.TrimPrefix(parsed.EscapedPath(), "/")
	name, err = url.PathUnescape(name)
	if err != nil || strings.TrimSpace(name) == "" || strings.Contains(name, "/") {
		return fmt.Errorf("DOCS_DATABASE_URL must contain one database name")
	}
	port := parsed.Port()
	if port == "" {
		port = "5432"
	...
	for key, value := range values {
		if err := os.Setenv(key, value); err != nil {
			return fmt.Errorf("set %s from DOCS_DATABASE_URL: %w", key, err)
		}
	}
	return nil
}
