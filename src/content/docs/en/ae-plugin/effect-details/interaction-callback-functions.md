...
While the un-macro'd function pointers are provided in [PF_InData](../../effect-basics/pf_indata#pf_indata), use the provided macros to access them. See how stringent we are about deprecating macro usage? Let's let this be our little secret.
...
| Function | Purpose |
|---|---|
| `PF_ADD_PARAM` | Enumerate your plug-in's parameters to After Effects during [PF_Cmd_PARAM_SETUP](../../effect-basics/command-selectors#global-selectors), using multiple calls to this function. |
| | !!! note |
...
| | Allocate a new [PF_ParamDef](../../effect-basics/pf_paramdef#pf_paramdef) to hold the result; those passed to the plug-in are read-only. |
...
| | Not doing so causes dismal performance and leaks memory. Once checked in, the fields in the [PF_ParamDef](../../effect-basics/pf_paramdef#pf_paramdef) will no longer be valid. |
...
The output from effect[n-1] is the input ([param[0]](../../effect-basics/pf_paramdef#param-zero)) of effect[n].
...
A 10fps layer checked out in a 30fps composition will only need to be refreshed every third frame. if your effect wants to change it's output every frame despite the static input layer, you'd need to set [PF_Outflag_NON_PARAM_VARY](../../effect-basics/pf_outdata#pf_outflags).
