...
After Effects passes effects an array of PF_ParamDefs with each selector, describing the plug-in's parameters at the current time. The values in the params array are only valid during some selectors (this is noted in the [selector descriptions](../command-selectors#calling-sequence)).
...
| `PF_ChangeFlags` | `change_flags` | Set if you've changed a parameter value. Only valid during drag (not click!) events, [PF_Cmd_USER_CHANGED_PARAM](../command-selectors#messaging) or [PF_Cmd_UPDATE_PARAMS_UI](../command-selectors#messaging). |
...
| [PF_ParamType](../parameters#parameter-types) | `param_type` | Type of parameter. |
...
| `PF_ParamDefUnion` | `u` | A union of all possible [Parameter Types](../parameters#parameter-types). |
...
| | This flag does not require that the [PF_OutFlag_CUSTOM_UI](../pf_outdata#pf_outflags) flag be set. |
| | If you want a standard control for `PF_Param_ARBITRARY_DATA`, just add one (or more) using `PF_PUI_STD_CONTROL_ONLY` with the supported param types, and then when handling [PF_Cmd_USER_CHANGED_PARAM](../command-selectors#messaging) you can modify your arb data. |
...
| `PF_PUI_DISABLED` | Disables (grays out) the parameter, usually in response to [PF_Cmd_USER_CHANGED_PARAM](../command-selectors#messaging). |
...
| | Premiere only: The flag is dynamic and parameter visibility can be toggled during the [PF_UpdateParamUI](../../effect-details/parameter-supervision#pf_paramutilsuite3) callback. |

In addition to these flags, an effect parameter may be hidden or shown by using [AEGP_GetDynamicStreamFlags](../../aegps/aegp-suites#aegp_dynamicstreamsuite4).

---

## Parameter Flags

Behavior flags and UI flags describe different qualities of a parameter. Set them *before* adding the parameter during [PF_Cmd_PARAM_SETUP](../command-selectors#global-selectors). Flags which may be set during events are noted.
...
| `PF_ParamFlag_COLLAPSE_TWIRLY` | Set this flag during [PF_Cmd_USER_CHANGED_PARAM](../command-selectors#messaging). |
| | This bit can now be set & cleared when handling [PF_Cmd_UPDATE_PARAMS_UI](../command-selectors#messaging) and [PF_Cmd_USER_CHANGED_PARAM](../command-selectors#messaging) messages, so as to twirl your parameters and groups up and down at will. |
| `PF_ParamFlag_SUPERVISE` | Set to receive [PF_Cmd_USER_CHANGED_PARAM](../command-selectors#messaging) messages for this parameter. |
...
| | Can be changed during parameter supervision, not just during [PF_Cmd_PARAM_SETUP](../command-selectors#global-selectors). |
| | This flag will not be honored unless [PF_OutFlag2_PARAM_GROUP_START_COLLAPSED](../pf_outdata#pf_outflags) is set. |
...
| `PF_ParamFlag_EXCLUDE_FROM_HAVE_INPUTS_CHANGED` | Only relevant if the effect sets [PF_OutFlag2_AUTOMATIC_WIDE_TIME_INPUT](../pf_outdata#pf_outflags) and will call [PF_AreStatesIdentical](../../effect-details/parameter-supervision#pf_paramutilsuite3) or [PF_HaveInputsChangedOverTimeSpan](../../effect-details/parameter-supervision#pf_paramutilsuite3) |
