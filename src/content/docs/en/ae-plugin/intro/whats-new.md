...
`AEGP_StreamSuite` is now at version 5, where [AEGP_GetExpression()](../../aegps/aegp-suites#aegp_streamsuite5) and [AEGP_SetExpression()](../../aegps/aegp-suites#aegp_streamsuite5) have been upgraded to support Unicode.

`PF_AdvTimeSuite` is now at version 4, with a new call [PF_TimeCountFrames()](../../effect-details/useful-utility-functions#pf_advtimesuite4), that returns the index of the frame in the current comp.
...
Comp Suite is now at version 11, with a new call, [AEGP_ReorderCompSelection()](../../aegps/aegp-suites#aegp_compsuite11), to move a selection to a certain layer index.
...
Unicode support for [AEGP Item Suite](../../aegps/aegp-suites#aegp_itemsuite9) and [AEGP Render Queue Item Suite](../../aegps/aegp-suites#render-queue-item-suite).
...
Starting in CC 2014, After Effects will now honor a change to a custom UI height made using [PF_UpdateParamUI](../../effect-details/parameter-supervision#pf_paramutilsuite3).

[AEGP Effect Suite](../../aegps/aegp-suites#aegp_effectsuite4) is now at version 4, adding new functions to work with effect masks. [AEGP_RenderSuite](../../aegps/aegp-suites#aegp_rendersuite4) is now at version 4, adding a new function `AEGP_RenderAndCheckoutLayerFrame`, which allows frame checkout of the current
...
[Mercury Transmit](../other-integration-possibilities#mercury-transmit) plug-ins and [HTML5 Panels](../other-integration-possibilities#html5-panels) are now supported.
...
We added the [PF_AngleParamSuite](../../effect-details/parameters-floating-point-values#pf_angleparamsuite1), providing a way to get floating point values for angle parameters. [PF App Suite](../../effect-details/useful-utility-functions) version 5 adds `PF_AppGetLanguage` to query the current language so that a plug-in can use the correct language string, as well as several new PF_App_ColorType enum values for new elements whose colors can be queried.

[AEGP Persistent Data Suite](../../aegps/aegp-suites#persistent-data-suite) is now at version 4, adding a new parameter to AEGP_GetApplicationBlob to choose between retrieving several different application blobs. There are also new functions to get/set time and ARGB values.

[AEGP Composition Suite](../../aegps/aegp-suites#aegp_compositesuite2) is now at version 10, adding new functions to check/modify whether layer names or source names are shown, and whether the blend modes column is shown or not. Also added are new functions to get and set the Motion Blur Adaptive Sample Limit.

[AEGP Layer Suite](../../aegps/aegp-suites#aegp_layersuite9) is now at version 8, adding new functions to set/get the layer sampling quality. [AEGP_CanvasSuite](../../artisans/artisan-data-types#aegp_canvassuite8) is also now at version 8. The new function `AEGP_MapCompToLayerTime` handles time remapping with collapsed or nested comps, unlike AEGP_ConvertCompToLayerTime.

[AEGP_UtilitySuite](../../aegps/aegp-suites#aegp_utilitysuite6) is now at version 6, adding a new Unicode-aware function: `AEGP_ReportInfoUnicode`. Another new function, `AEGP_GetPluginPaths`, provides some useful paths related to the plug-in and the After Effects executable itself.
...
[AEGP_IOInSuite](../../aeios/new-kids-on-the-function-block#aegp_ioinsuite5) is now at version 5, adding new functions to get/set/clear the native start time, and to get/set the drop-frame setting of footage.
...
The new Global Performance Cache means you must tell After Effects to discard old cached frames [when changing your effect's rendering](../../effect-details/tips-tricks#caching-behavior).

We've removed `PF_HasParamChanged` and `PF_HaveInputsChangedOverTimeSpan`, providing [PF_AreStatesIdentical](../../effect-details/parameter-supervision#pf_paramutilsuite3) instead.
...
3D is a major theme of AE CS6. A new `AEGP_LayerFlag_ENVIRONMENT_LAYER` has been added. Many new [layer streams](../../aegps/aegp-suites#aegp_streamsuite5) were added.
...
A new suite, [AEGP_RenderQueueMonitorSuite](../../aegps/aegp-suites#render-queue-monitor-suite), provides all the info a render queue manager needs to figure out what is happening at any point in a render.

[AEGP Mask Suite](../../aegps/aegp-suites#aegp_masksuite6) is now at version 6, and provides functions to get and set the mask feather falloff type. [AEGP Mask Outline Suite](../../aegps/aegp-suites#aegp_maskoutlinesuite3) is now at version 3, and provides access to get and set mask outline feather information.

Effects that depend on masks now have a new flag available, `PF_OutFlag2_DEPENDS_ON_UNREFERENCED_MASKS`.

[AEGP Composition Suite](../../aegps/aegp-suites#aegp_compositesuite2) is now at version 9. AEGP_CreateTextLayerInComp and

AEGP_CreateBoxTextLayerInComp now have a new parameter, select_new_layerB.

[AEGP Render Suite](../../aegps/aegp-suites#aegp_rendersuite4) is now at version 3, adding a new function to get the GUID for a render receipt.

Finally, we have added two new read-only [Dynamic Stream](../../aegps/aegp-suites#aegp_dynamicstreamsuite4) flags: `AEGP_DynStreamFlag_SHOWN_WHEN_EMPTY` and `AEGP_DynStreamFlag_SKIP_REVEAL_WHEN_UNHIDDEN`.
