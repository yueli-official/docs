...
Returns the color of the environment texture in the direction given by the `reflect_dir` argument. The signatures that specify four vectors filter the map pixels found within a cone defined by the vectors. Instead of using a cone, you can also define an angle to filter over the map, using the `filter_angle` argument with a single reflection direction.

You can also use the environment function to look up intensity values in an IES photometric light map with the correct angular mapping applied. To normalize the IES data, set `iesnormalization` mode to

- `preserveenergy`: preserves the light’s energy output
- `maxvalue`: uses maximum value in the table for normalization
- `none`: no normalization

Note
To perform environment map lookups in object space, you must first transform the direction vector with the `vtransform()` function.
