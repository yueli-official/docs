...
`pathtrace` works like [irradiance](/en/houdini-vex/shading-and-rendering/irradiance "Computes irradiance (global illumination) at the point P with the normal N."), but uses the physically based
...
Irradiance caching works the same way it works with [occlusion](/en/houdini-vex/shading-and-rendering/occlusion "Computes ambient occlusion.").
Ray options
