...
| `outMaxBufferSize` | The maximum audio buffer size needed if the transmitter uses plugin-based audio to request audio buffers using the [Playmod Audio Suite](../suites#playmod-audio-suite). |
...
| | The audio output names in tmAudioMode should be allocated by the plugin using the [String Suite](../../universals/sweetpea-suites#string-suite) and NOT disposed by the plugin. The host will take care of disposing these strings. |
