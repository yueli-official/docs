...
The previous frames of the input clip are combined with
the current frame to give a variety of 'time trails' effects. The
output of each processed frame is stored and then combined with the
next frame. The trails are reinitialized whenever any non-consecutive
frame is processed, either the first frame, reprocessing a given
frame, or jumping to another frame. You must process multiple
frames of a clip in a row to observe the effect, and clearing your
image cache before rendering may sometimes be necessary.

In the Sapphire Time effects submenu.

![Trails](../_static/Trails.jpg)
