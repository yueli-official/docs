...
- **Matte**: Defaults to None. If provided, the effect is applied only at Source areas specified by this input. For gray values in the matte, the pixel blocks are mixed with the original source such that the blocks fade but remain whole. This input can be affected using the Blur Matte, Invert Matte, or Matte Use parameters.
