...
- To respond to changes in the status of a particular item while the render is progressing, define a callback function in [RenderQueueItem.onstatus](../renderqueueitem#renderqueueitemonstatus) in the associated RenderQueueItem object.
...
Stops the rendering process. This is the same as clicking Stop in the Render Queue panel during a render. You can call this method from an [RenderQueueItem.onstatus](../renderqueueitem#renderqueueitemonstatus) or [app.onError](../../general/application#apponerror) callback.
