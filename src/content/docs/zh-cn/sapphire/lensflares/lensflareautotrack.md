...
在背景素材上渲染一个或多个镜头光晕图像，将各种光晕元素在热点和枢轴位置之间对齐。在此 AutoTrack 版本的 LensFlare 中，热点会自动定位在背景素材的最亮区域。增加 Blur For Auto 会在查找最亮位置之前对输入进行平滑处理，有助于消除次要亮点的影响。
...
- **Track**: 默认为无。镜头光晕热点定位在此图像的最亮部分。如果未选择此输入，则也使用背景输入（当前图层）进行跟踪。
...
*Check-box, Default:
*on.
打开或关闭用于调整 Pivot 参数的屏幕用户界面。此参数仅出现在 AE 和 Premiere 上，这些平台支持屏幕控件。