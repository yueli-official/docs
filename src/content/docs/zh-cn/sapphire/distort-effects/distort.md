...
使用 Lens 输入片段的梯度对源输入片段进行扭曲变形。这可以生成光学玻璃般的效果，就像透过任意形状的透镜观看源片段一样。当透镜图像仅包含几个粗体形状或简单纹理时，效果最为明显。

在 Sapphire Distort 效果子菜单中。

![Distort](../_static/Distort.jpg)
...
- **Mask Lens** (Check-box, Default: off)
  如果启用，除了源输入外，还将遮罩应用于 Lens 输入。

- **Amount** (Default: 1, Range: any)
  透镜扭曲的强度。设为负值可反转扭曲方向。

- **Fine** (Check-box, Default: off)
  如果启用，扭曲量将减少 100 倍。此模式旨在允许在作为 Lens 输入给出的遮罩边缘附近对源图像进行微妙的扩展或收缩。
