...
使用 Lens 输入片段的梯度对源输入的红色、绿色和蓝色颜色通道进行不同程度的扭曲。当透镜图像仅包含几个简单的粗体形状时，效果最为明显。

在 Sapphire Distort 效果子菜单中。

![DistortRGB](../_static/DistortRGB.jpg)
...
- **Warp Red** (Default: 0.5, Range: any)
  缩放红色通道的透镜扭曲量。取负值可反转方向。

- **Warp Green** (Default: 0.75, Range: any)
  缩放绿色通道的透镜扭曲量。取负值可反转方向。

- **Warp Blue** (Default: 1, Range: any)
  缩放蓝色通道的透镜扭曲量。取负值可反转方向。
