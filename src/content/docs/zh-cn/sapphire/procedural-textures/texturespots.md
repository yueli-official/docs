...
创建可以扭曲和动画的斑点图案。Warp Speed 参数使斑点随时间被随机扭曲图案扭曲。

在 Sapphire Render 效果子菜单中。

![TextureSpots](../_static/TextureSpots.jpg)
...
- **Radius** (Default: 1, Range: 0 to 2)
  斑点的半径。调整此参数可在不改变斑点数量的情况下更改斑点大小。
...
- **Rand Warp Freq** (Default: 1, Range: 0.01 or greater)
  用于扭曲变形的噪波空间频率。除非 Rand Warp Amp 为正值，否则无效。

- **Shift** (X & Y, Default: [0 0], Range: any)
  纹理的平移偏移。由于纹理是程序化生成的，可以平移而不会出现重复单元或接缝。

- **Warp Start** (X & Y, Default: [0 0], Range: any)
  扭曲图案的平移偏移。除非 Rand Warp Amp 为正值，否则无效。

- **Warp Speed** (X & Y, Default: [0.5 0], Range: any)
  扭曲图案的平移速度。如果非零，斑点将以此速率进行摆动动画。除非 Rand Warp Amp 为正值，否则无效。
