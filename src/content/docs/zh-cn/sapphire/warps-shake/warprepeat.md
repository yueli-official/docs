...
对源输入进行多次变换并将结果取平均。From 和 To 参数不指时间。它们描述空间中的两种变换，决定应用于每帧的重复变形序列。
...
- **Steps** (Integer, Default: 3, Range: 2 to 100)
  沿 From 和 To 变换之间路径采样输入图像的次数。更多步骤需要更多处理时间。
...
- **Exposure Bias** (Default: 0.5, Range: 0 to 1)
  确定沿 From 和 To 变换之间路径的可变曝光量。值为 0 使 From 端的曝光更多，0.5 使路径上的曝光均匀，1.0 使 To 端的曝光更多。如果在暗背景上有亮点，值为 0 会使处理后的亮点在 From 端更亮、在 To 端更暗，值为 1.0 则效果相反。
