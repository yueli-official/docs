...
将源素材变形为圆形盘状形状。源图像的垂直方向映射在 Inner Radius 和 Outer Radius 之间，水平方向根据 Angle Repeats 的数量围绕中心旋转，并按 Angle 偏移。

在 Sapphire Distort 效果子菜单中。

![WarpPolar](../_static/WarpPolar.jpg)
...
- **Angle** (Default: 0, Range: any)
  结果的旋转角度（以度为单位）。此参数可以使用 Angle 控件调整。

- **Angle Repeats** (Default: 1, Range: 0.01 or greater)
  围绕一周的源图像副本数量。这应该是整数以避免第一个副本与最后一个副本连接处出现接缝。

- **Center** (X & Y, Default: [0 0], Range: any)
  圆盘的中心，在屏幕坐标中相对于画面中心。

- **Stretch** (X & Y, Default: [1 1], Range: 0.01 or greater)
  缩放圆盘形状的水平或垂直大小。此参数可以使用 Inner Radius 控件调整。

- **Inner Radius** (Default: 0.028, Range: any)
  源素材底部边缘映射到的距中心距离。此参数可以使用 Inner Radius 控件调整。

- **Outer Radius** (Default: 0.84, Range: any)
  输入素材顶部边缘映射到的距中心距离。此参数可以使用 Outer Radius 控件调整。

- **Wrap** (X & Y, Popup menu, Default: [ Tile No ])
  确定访问源图像边界外区域的方法。
  - **No**: 在边界外显示黑色。
  - **Tile**: 重复图像的副本。
...
- **Show Angle** (Check-box, Default: on)
  打开或关闭用于调整 Angle 参数的屏幕用户界面。此参数仅在 AE 和 Premiere 中出现，支持屏幕控件。
