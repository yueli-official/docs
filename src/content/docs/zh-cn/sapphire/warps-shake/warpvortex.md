...
围绕给定的 Center 位置将源素材扭曲为漩涡状。使用 Vortex Start 参数调整漩涡量，使用 Angle Offset 同时应用普通旋转。Vortex Speed 可用于自动动画化漩涡量。
...
- **Vortex Start** (Default: 36, Range: any)
  漩涡旋转量，在画面边缘处的近似角度（以度为单位）。

- **Vortex Speed** (Default: 0, Range: any)
  漩涡旋转速度，在画面边缘处的近似角度（每秒度数）。如果非零，漩涡将以此速率自动动画化。

- **Angle Offset** (Default: 0, Range: any)
  如果非零，将旋转与漩涡组合。设为负值可使内部和外部区域向相反方向旋转。
...
- **Inner Radius** (Default: 0.042, Range: 0 or greater)
  从中心开始引入漩涡效果的半径。可用于减少漩涡中心处的过度变形和锯齿。
