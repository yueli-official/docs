...
根据源素材的梯度对其进行变形。默认情况下，较亮的区域会膨胀，较暗的区域会收缩。这类似于使用 Distort 效果将图像用作自身的镜头。

在 Sapphire Distort 效果子菜单中。

![WarpPuff](../_static/WarpPuff.jpg)
...
- **Direction** (Popup menu, Default: Puff)
  确定源素材的哪种类型的区域被膨胀。
  - **Puff**: 扩展较亮的区域，收缩较暗的区域。
  - **Shrivel**: 收缩较亮的区域，扩展较暗的区域。

- **Amount** (Default: 0.5, Range: any)
  缩放变形量。也可以为负值，将膨胀变为收缩，反之亦然。

- **Smoothness** (Default: 0.4, Range: 0 or greater)
  在确定变形方向和量之前按此量模糊源素材。

- **Rotate Warp Dir** (Default: 0, Range: any)
  旋转变形方向。这可以使亮度相似的区域产生扭曲效果，而不仅仅是扩展或收缩。
