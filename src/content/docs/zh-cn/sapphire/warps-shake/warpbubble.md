...
通过平滑噪波函数对源素材进行变形。可用于创建热扩散或水下类型的效果。Shift Speed 参数使噪波图案随时间自动平移。调整 Amplitude 和 Frequency 参数以获得不同类型的变形效果。
...
- **Frequency** (Default: 16, Range: 0.01 or greater)
  噪波图案的频率。增大以获得更多更小的气泡，减小以获得更少更大的气泡。

- **Frequency Rel X** (Default: 1, Range: 0.01 or greater)
  气泡图案的相对水平频率。增大以获得更高的气泡，减小以获得更宽的气泡。
...
- **Shift Speed** (X & Y, Default: [0.1 0], Range: any)
  如果非零，气泡图案将以此速度自动动画平移。动画化 Speed 值的结果可能不太直观，因此对于变速运动，通常最好将此设置为 0 并改为对 Shift Start 值设置动画。
