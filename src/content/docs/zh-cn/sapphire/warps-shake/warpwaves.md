...
通过波纹图案对源素材进行变形。可以通过增大 Phase Speed 参数或对 Phase Start 值设置动画来使波纹随时间移动。
...
- **Frequency** (Default: 8, Range: 0.1 or greater)
  波纹的频率。增大以获得更多波纹，减小以获得更少波纹。

- **Angle** (Default: 45, Range: any)
  波纹图案的旋转角度（以度为单位）。如果角度为 0，波纹向右移动并垂直排列。

- **Displace Angle** (Default: 90, Range: any)
  相对于波纹角度的变形方向（以度为单位）。0 产生压缩-扩展波纹，90 产生左右摆动波纹。

- **Phase Start** (Default: 0, Range: any)
  波纹的相位偏移。波纹图案沿 Angle 方向按此量平移。

- **Phase Speed** (Default: 0, Range: any)
  波纹的相位速度。如果非零，波纹图案将以此速率自动移动。
