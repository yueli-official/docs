...
通过同心波图案对源素材进行变形。Phase Speed 参数使波纹随时间自动从中心向外移动。调整 Inner 和 Outer Radius 参数以限制波纹出现的区域。增大 Inner 和 Outer softness 以获得波纹出现和不出现区域之间更平滑的过渡。
...
- **Rotate Puddle** (Default: 0, Range: any)
  在应用 Rel Height 拉伸后旋转水波图案指定的角度（以度为单位）。当 Rel Height 为 1 时此参数无效。此参数可以使用 Rotate Puddle 控件调整。

- **Center** (X & Y, Default: [0 0], Range: any)
  水波的中心，在屏幕坐标中相对于画面中心。此参数可以使用 Center 控件调整。
...
- **Show Rotate Puddle** (Check-box, Default: on)
  打开或关闭用于调整 Rotate Puddle 参数的屏幕用户界面。此参数仅在 AE 和 Premiere 中出现，支持屏幕控件。
