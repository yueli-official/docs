...
Finds the edges of the source input that are aligned in a specified
...
- **Direction** (X & Y, Default: [0.389 0.361], Range: any)
  Edges are found which are perpendicular to this direction vector. This parameter can be adjusted using the Direction Widget.
...
- **Show Direction** (Check-box, Default: on)
  Turns on or off the screen user interface for adjusting the Direction parameter.This parameter only appears on AE and Premiere, where on-screen widgets are supported.
