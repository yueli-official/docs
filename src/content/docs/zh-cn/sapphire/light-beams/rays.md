...
从源素材的明亮区域生成光束。降低 Threshold 参数可从更多区域生成光线，提高该参数则仅从最亮的区域生成光线。将 Rays Res 参数设置为 1/2 可获得更快的渲染速度，光线会略微柔和一些。
...
- **Threshold** (Default: 0.5, Range: 0 or greater)
  从源素材中亮度高于此值的位置生成光线。值为 0.9 时仅在最亮的位置产生光线。值为 0 时每个非黑色区域都会产生光线。

- **Threshold Add Color** (Default rgb: [0 0 0])
  可用于提高特定颜色的阈值，从而减少源素材中包含该颜色的区域所生成的光线。
...
- **Rays From Alpha** (Default: 0, Range: 0 to 1)
  设置为 1 可从源素材的 Alpha 通道而非 RGB 通道生成光线。这通常会导致生成更多的光线。0 到 1 之间的值在使用 RGB 和 Alpha 之间进行插值。
...
- **Use Source Chroma** (Default: 1, Range: 0 or greater)
  如果此值为 1，源输入的色度会影响生成光线的色度。如果为 0，仅源输入的亮度影响光线的亮度，渲染速度也应更快。0 到 1 之间的值在这两个选项之间进行插值。
