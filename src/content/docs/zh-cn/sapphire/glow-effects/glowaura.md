...
![GlowAura](../_static/GlowAura.jpg)


### Inputs:

- **Source**: The current layer. The input clip that determines the glow locations and directions.
...
- **Outer Brightness** (Default: 1, Range: 0 or greater)
  Scales the brightness of the glows at further distances from the source.

- **Glow Saturation** (Default: 1, Range: any)
...
- **Threshold** (Default: 0, Range: 0 or greater)
  Glows will be generated from locations in the source clip that are brighter than this value. A value of 0.9 causes glows at only the brightest spots. A value of 0 causes glows on every non-black area.
...
- **Glow Width** (Default: 0.4, Range: 0 or greater)
  Scales the glow distance. This parameter can be adjusted using the Width Widget. Note that a zero glow width still enhances the bright areas; set the brightness parameter to zero if you want to pass the Source through unchanged.
