...
将图像分割成圆形瓦片，并对每个形状内的图像进行变换，以创建蝇眼视觉效果。重叠选项允许在圆形重叠处以不同方式组合。"Inside"参数在源图像被平铺到图案之前对其进行变换，"Tile"参数则变换整个蝇眼图案。
...
- **Circle Radius** (Default: 1, Range: 0 to 1)
  圆形相对于彼此的半径。如果小于 1.0，圆形之间会出现空白区域。这些空白区域的颜色将是透明、黑色或白色，具体取决于组合模式。

- **Edge Softness** (Default: 0, Range: 0 to 1)
  圆形边缘的柔和度。如果增大此值，可能还需要降低 Circle Radius 以避免在柔和边缘重叠处出现矩形伪影。
