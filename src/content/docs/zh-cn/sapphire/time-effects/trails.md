...
输入素材的先前帧与当前帧组合，产生多种"时间拖尾"效果。每个处理后帧的输出会被存储，然后与下一帧组合。每当处理任何非连续帧时，拖尾都会重新初始化：包括第一帧、重新处理某一帧或跳转到另一帧。您必须连续处理素材的多个帧才能观察到效果，渲染前清除图像缓存有时可能是必要的。
...
- **Blur Rel** (X & Y, Default: [1 1], Range: 0 or greater)
  模糊的相对水平和垂直量。除非 Blur Amount 为正数，否则不起作用。
