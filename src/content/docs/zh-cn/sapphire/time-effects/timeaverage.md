...
每个输出帧是多个输入帧的平均值：从当前帧回溯到给定数量的先前帧。这与 Trails 效果类似，但范围内的所有帧权重相等而非逐渐淡出，因此拖尾的端点是突然的。每一帧仅贡献总亮度的 1/n，因此在暗背景上快速移动的对象可能看起来较暗。

当处理任何非连续帧时，平均值会重新初始化：包括第一帧、重新处理某一帧或跳转到另一帧。您必须连续处理素材的多个帧才能观察到效果，渲染前清除图像缓存有时可能是必要的。此效果在场模式下无法正常工作。请确保在帧模式下处理。
...
- **Ave Over Frames** (Integer, Default: 10, Range: 1 or greater)
  要平均的先前帧数，包括当前帧。为获得正确结果，此参数不应设置动画。
