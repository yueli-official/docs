---
title: FeedbackDistort
---

## S_FeedbackDistort

输入素材的先前帧通过给定 Lens 输入素材的梯度进行扭曲，并与当前帧组合以产生多种可能的效果。每个处理后帧的输出会被存储，然后与下一帧组合。每当处理任何非连续帧时，反馈都会重新初始化：包括第一帧、重新处理某一帧或跳转到另一帧。您必须连续处理素材的多个帧才能观察到效果，渲染前清除图像缓存有时可能是必要的。

在 Sapphire Time effects 子菜单中。

![FeedbackDistort](../_static/FeedbackDistort.jpg)
...
- **Blur Lens** (Default: 0.08, Range: 0 or greater)
  在使用前按此数值平滑 Lens 图像。增大此值可获得更平滑、更连续的反馈方向。

- **Rotate Distort** (Default: 90, Range: any)
  将扭曲方向旋转此角度数。如果非零，可产生一些扭转扭曲效果。
...
- **Show Center** (Check-box, Default: off)
  开启或关闭用于调整 Center 参数的屏幕用户界面。此参数仅在 AE 和 Premiere 中出现，这些宿主支持屏幕小工具。
