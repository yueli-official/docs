...
从 FromObj 输入片段中物体边缘向外生成多条闪电，并将其渲染在背景输入上。使用 Show:Edges 选项可以在调整 Threshold 和 Blur From Obj 参数时查看源边缘。
...
- **FromObject**: 默认为无。提取此片段中物体的边缘，闪电从这些边缘上的点开始。

- **Matte**: 默认为无。如果提供，每个区域的闪电长度将按此输入缩放。白色区域生成正常闪电，灰色区域生成较短的闪电，黑色区域不生成闪电。
...
- **Surface Bolts** (Integer, Default: 25, Range: 1 to 2000)
  沿边缘生成闪电的点数。这些表面闪电按 FromObject 输入中各个独立形状的大小比例分配。
...
- **Blur FromObj** (Default: 0.088, Range: 0 or greater)
  在查找边缘之前模糊 FromObject 输入片段。这有助于去除噪点并减少独立形状的数量。您可以在调整此参数时使用 Show Edges 选项直接查看边缘图像。

- **FromObj Use** (Popup menu, Default: Luma)
  确定使用 FromObject 输入的哪个通道。
  - **Luma**: 使用 RGB 通道的亮度。
  - **Alpha**: 仅使用 Alpha 通道。
...
- **Show** (Popup menu, Default: Result)
  选择输出选项。
  - **Result**: 显示背景上的正常闪电结果。
  - **Edges**: 显示边缘图像。在调整 Threshold 和 Blur From Obj 参数时查看此图像很有用。

- **Max Length** (Default: 0.2, Range: 0 or greater)
  缩放闪电的长度。

- **Vary Length** (Default: 0.5, Range: 0 to 1)
  每条闪电长度的随机变化量。值为 0 时所有闪电长度等于 Max Length，值为 1 时闪电长度在零和 Max Length 之间变化。

- **Vary Spacing** (Default: 0.5, Range: 0 or greater)
  每条闪电沿边缘起始点的随机变化量。值为 0 时闪电均匀分布，值为 1 时闪电随机分布。
