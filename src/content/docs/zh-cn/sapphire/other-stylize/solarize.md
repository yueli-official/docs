...
反转输入素材中亮于 Threshold 值的颜色，以创建"曝光过度"效果。

在 Sapphire Stylize 效果子菜单中。

![Solarize](../_static/Solarize.jpg)
...
- **Threshold** (Default: 0.5, Range: 0 or greater)
  高于此值的颜色将被反转。如果为 0，所有颜色都被反转以产生负片效果。如果为 1，没有颜色被反转，结果应与输入相同。

- **Saturation** (Default: 1, Range: any)
  缩放结果的颜色饱和度。增加可获得更鲜艳的颜色。设为 0 可获得单色效果。也可以将此值设为负数以反转结果的色度。

- **Invert** (Check-box, Default: off)
  如果启用，反转结果。反转在使用 Brightness 和 Offset 之前应用，因此更改反转选项时可能需要重新调整这些参数。

- **Brightness** (Default: 2, Range: 0 or greater)
  缩放结果的亮度。注意，如果使用 0.5 的 Threshold，没有颜色会超过最大亮度的一半，因此通过将 Brightness 设为 2 来增加对比度。

- **Offset Darks** (Default: 0, Range: any)
  在曝光过度效果后向较暗区域添加此灰度值。可以为负值以增加对比度。
