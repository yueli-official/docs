...
使用一系列条纹在两个输入素材之间执行擦除转场。应对 Wipe Percent 参数设置动画以控制转场速度。增大 Grad Add 参数可使条纹图案的时序在擦除过程中移动到屏幕上。增大 Border Width 参数可在擦除转场边缘绘制边框。
...
- **Angle** (Default: 45, Range: any)
  用于擦除的整体条纹图案的旋转角度（以度为单位）。

- **Frequency** (Default: 6, Range: 0.1 or greater)
  条纹图案的频率。增大可获得更多更小的元素，减小可获得更少更大的元素。
