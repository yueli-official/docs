...
使用不断增大或缩小的圆形在两个输入素材之间执行擦除转场。应对 Wipe Percent 参数设置动画以控制转场速度。增大 Border Width 参数可在擦除转场边缘绘制边框。
...
- **Wipe Direction** (Popup menu, Default: Circle In)
  圆形擦除的方向。
  - **Circle In**: 圆形包含第一个图像并向内缩小。
  - **Circle Out**: 圆形包含第二个图像并向外扩大。

- **Edge Softness** (Default: 0, Range: 0 or greater)
  转场边缘的宽度。较大的值将导致擦除图案中更柔和、更不明显的边缘。

- **Rel Width** (Default: 1, Range: 0.1 or greater)
  "圆形"形状的相对宽度。增大可获得更宽的椭圆，减小可获得更高的椭圆。

- **Rotate** (Default: 0, Range: any)
  "圆形"的旋转角度（以度为单位）。如果 Rel Width 参数为 1.0，则此参数无效。

- **Center** (X & Y, Default: [0 0], Range: any)
  圆形中心在屏幕坐标中相对于画面中心的位置。可以通过启用和移动 Center Widget 来设置此参数。请注意，移动圆形中心也会导致圆形大小改变，以使 Wipe Amt 的当前值保持正确。
