...
在两个输入素材之间执行简单的线性擦除转场。应对 Wipe Percent 参数设置动画以控制转场速度。增大 Border Width 参数可在擦除转场边缘绘制边框。
...
- **Show Wipe** (Check-box, Default: on)
  打开或关闭用于调整 Wipe Amt、Angle 和 Edge Softness 参数的屏幕用户界面控件。此参数仅在 AE 和 Premiere 中显示，因为这些软件支持屏幕控件。
