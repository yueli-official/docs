...
使用不断增大或缩小的六边形、三角形、菱形或星形图案在两个输入素材之间执行擦除转场。应对 Wipe Percent 参数设置动画以控制转场速度。增大 Grad Add 参数可使瓦片图案的时序在擦除过程中移动到屏幕上。增大 Border Width 参数可在擦除转场边缘绘制边框。
...
- **Tile Shapes** (Popup menu, Default: Hexagons)
  用于生成图案的瓦片形状。请注意，Morph Shapes 参数可以将形状从此设置进行变换。
  - **Hexagons**: 蜂巢状的六边形图案
  - **Triangles**: 三角形网格
  - **Diamonds**: 菱形图案
  - **Stars**: 六角星合并形成正方形图案。
