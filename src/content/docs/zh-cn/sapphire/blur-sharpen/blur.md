...
使用高斯、三角或方框滤波器按任意量模糊源素材。即使 Width 值非常大，此效果也能快速渲染。使用 Blur Rel X 和 Y 参数可获得更水平或更垂直的模糊方向。

位于 Sapphire Blur+Sharpen 效果子菜单中。

![Blur](../_static/Blur.jpg)
...
- **Scale Opacity** (Default: 1, Range: 0 or greater)
  缩放结果的不透明度（Alpha 通道）。
...
- **Subpixel** (Check-box, Default: on)
  启用亚像素量的模糊。使用此选项可使 Blur Amount 或 Blur Rel 参数的动画更平滑。
