...
- **Show Kernel Size** (Check-box, Default: on)
  Turns on or off the screen user interface for adjusting the Kernel Size parameter.This parameter only appears on AE and Premiere, where on-screen widgets are supported.
