---
title: FlickerRemoveMatte
---

## S_FlickerRemoveMatte

使用遮罩片段指定平均亮度应保持恒定的区域，从源片段中移除时间上的闪烁。要使用此效果，选择遮罩内具有所需亮度的源帧，并点击 Set Hold Level 按钮。处理其他帧时，其亮度将被缩放，使遮罩内的平均亮度等于保持级别。您可以随时间对不同的 Hold Level 值设置关键帧，以适应所需的亮度变化。

在 Sapphire Time 效果子菜单中。

![FlickerRemoveMatte](../_static/FlickerRemove.jpg)
...
- **Matte**: 默认为无。此片段指定从哪些源区域测量闪烁。如果未提供此输入，则使用源输入的 Alpha 通道作为遮罩。可以使用 Invert Matte 参数进行反转。
...
- **Combine Masks** (Popup menu, Default: Union)
  确定当两个遮罩都提供给效果时，如何组合 Mocha 遮罩和输入遮罩。
  - **Union**: 使用两个遮罩共同覆盖的区域。
...
- **Set Hold Level** (Push-button)
  按下此按钮会将 Hold Level 参数设置为当前帧遮罩内源片段的平均亮度。这会使输出在此帧等于源。此按钮本身不保留任何值，按下后会立即恢复关闭状态。
