...
使用第二个匹配片段的颜色变化，在遮罩指定的区域内为源片段添加颜色变化。要使用此效果，选择一帧您希望源颜色保持不变的帧，并点击 Set Match Color 按钮。处理其他帧时，源颜色将按遮罩内匹配片段的平均颜色相对于匹配颜色进行缩放。
...
- **Match Color** (Default rgb: [0.5 0.5 0.5])
  遮罩内匹配片段的平均颜色，在此颜色下源输入保持不变。

- **Set Match Color** (Push-button)
  按下此按钮会将 Match Color 参数设置为当前帧遮罩内匹配片段的平均颜色。这会使输出在此帧等于源。此按钮本身不保留任何值，按下后会立即恢复关闭状态。
