...
在必要时降低输入素材的色度，使其不超过指定的最大值。此效果可用于制作"广播安全"颜色。它还可以用于缩放色度、钳制亮度或缩放亮度。
...
- **Clamp Chroma** (Default: 0.5, Range: 0 to 1)
  最大色度值。1 为完全饱和，0 为无颜色。低于此值的源色度不会受到影响，但高于此值的将被降低到此值。

- **Scale Chroma** (Default: 1, Range: 0 or greater)
  缩放所有像素的色度。如果为 1 则无效果。

- **Clamp Luma** (Default: 1, Range: 0 to 1)
  最大亮度值。亮度高于此限制的源像素将被降低到此值。低于此值的不受影响。如果为 1 则无效果。

- **Scale Luma** (Default: 1, Range: 0 or greater)
  缩放所有像素的亮度。如果为 1 则无效果。
