...
export async function serveDiscoveryArtifact(event: H3Event, name: string): Promise<string> {
  if (name.length === 0 || name.includes('\\') || name.includes('..') || name.startsWith('/')) {
    throw createError({ statusCode: 400, statusMessage: 'Invalid discovery artifact' })
  }
  ...
  if (result?.contractVersion !== 'discovery.v1') {
    throw createError({ statusCode: 502, statusMessage: 'Unsupported discovery publication' })
  }
  const artifact = result.artifact
  if (artifact === undefined) {
    throw createError({ statusCode: 404, statusMessage: 'Discovery artifact not found' })
  }
  const encodedBytes = new TextEncoder().encode(artifact.content).byteLength
  if (encodedBytes !== artifact.bytes) {
    throw createError({ statusCode: 502, statusMessage: 'Invalid discovery artifact length' })
  }
  const digest = createHash('sha256').update(artifact.content, 'utf8').digest('hex')
  if (digest !== artifact.sha256.toLowerCase()) {
    throw createError({ statusCode: 502, statusMessage: 'Invalid discovery artifact digest' })
  }
