...
const { ensure, tree, collection, pending } = useCollectionTree(
    ...
    <div class="mx-auto flex w-full max-w-[1400px] flex-1 gap-8 px-4 py-8">
      <aside class="hidden w-64 shrink-0 lg:block">
        ...
        >
          <NuxtLink
            :to="collectionTo"
            class="mb-4 flex min-w-0 items-center gap-3 rounded-lg border border-default bg-default p-2 text-highlighted transition hover:border-primary/40 hover:text-primary"
          >
            <span
              class="relative size-10 shrink-0 overflow-hidden rounded-md bg-elevated"
            >
              <img
            ...
            <span class="min-w-0">
              <span
                class="line-clamp-2 font-display text-sm font-semibold leading-snug"
                >{{ collection?.title || slug }}</span
              >
              <span class="mt-0.5 block text-xs font-normal text-muted"
                >{{ tree.length }} 个顶级章节</span
              >
            </span>
          </NuxtLink>
      ...
      <main id="public-main" tabindex="-1" class="min-w-0 flex-1 outline-none">
        <div
          v-if="currentPath"
          class="mb-5 flex items-center justify-between gap-3 rounded-xl border border-default bg-elevated/35 px-3 py-2 lg:hidden"
        >
