...
const { data, pending, error, refresh } = await useAsyncData(
  ...
  { server: false, default: () => ({ batch: null as unknown as DocsImportBatch, items: [] as DocsImportItem[] }) },
)
...
function statusMeta(status?: string) {
  if (status === 'completed') return { label: '已完成', color: 'success' as const, icon: 'i-tabler-circle-check' }
  if (status === 'rolled_back') return { label: '已回滚', color: 'neutral' as const, icon: 'i-tabler-history' }
  if (status === 'failed') return { label: '失败', color: 'error' as const, icon: 'i-tabler-alert-circle' }
  if (status === 'checked') return { label: '已预检', color: 'warning' as const, icon: 'i-tabler-clipboard-check' }
  if (status === 'running') return { label: '执行中', color: 'primary' as const, icon: 'i-tabler-loader-2' }
  return { label: status || '未知', color: 'neutral' as const, icon: 'i-tabler-file-import' }
}

function actionMeta(action: string) {
  const map: Record<string, { label: string; color: 'primary' | 'neutral' | 'warning' | 'error'; icon: string }> = {
    ...
    skip: { label: '跳过', color: 'neutral', icon: 'i-tabler-player-skip-forward' },
    conflict: { label: '冲突', color: 'error', icon: 'i-tabler-alert-triangle' },
    error: { label: '错误', color: 'error', icon: 'i-tabler-alert-circle' },
  }
  return map[action] ?? { label: action, color: 'neutral', icon: 'i-tabler-circle' }
}
...
async function rollback() {
  ...
  try {
    ...
    toast.add({ title: '回滚失败', description: err?.data?.message || '请稍后重试', color: 'error' })
  }
  finally {
...
<template>
  <div>
    ...
    <div v-else-if="loadError" class="rounded-lg border border-default bg-default p-8">
      <div class="mx-auto max-w-md text-center">
        <span class="mx-auto grid size-12 place-items-center rounded-lg bg-error/10 text-error">
          <UIcon name="i-tabler-alert-circle" class="size-6" />
        </span>
        <h2 class="mt-4 text-lg font-semibold text-highlighted">导入批次加载失败</h2>
        <p class="mt-2 text-sm leading-6 text-muted">{{ loadError.data?.message || loadError.message || '请稍后重试' }}</p>
        <UButton class="mt-5" icon="i-tabler-refresh" label="重试" color="neutral" variant="soft" @click="() => refresh()" />
      </div>
    </div>

    <template v-else-if="batch">
      <section class="mb-4 overflow-hidden rounded-lg border border-default bg-default">
        <div class="border-b border-default bg-elevated/35 px-5 py-4">
          <div class="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p class="mt-1 text-sm text-muted">模式 {{ batch.mode }} · 默认语言 {{ batch.defaultLocale }}</p>
            </div>
            <UBadge
      ...
      <div class="grid gap-4 xl:grid-cols-[minmax(0,1fr)_340px]">
        <section class="min-w-0 overflow-hidden rounded-lg border border-default bg-default">
          <div class="hidden grid-cols-[110px_minmax(220px,1fr)_150px_180px] gap-3 border-b border-default bg-elevated/45 px-4 py-2.5 text-xs font-medium text-muted lg:grid">
            <span>动作</span>
            <span>文档</span>
            <span>语言 / 版本</span>
            <span>源文件</span>
          ...
          <div v-else class="divide-y divide-default">
            ...
            >
              ...
              />
              <div class="min-w-0">
                <p class="truncate text-sm font-medium text-highlighted">{{ item.title || item.path }}</p>
                <p class="truncate font-mono text-xs text-muted">{{ item.path }}</p>
              ...
              <p class="font-mono text-xs text-muted">{{ item.locale }} / {{ item.versionKey }}</p>
              <p class="truncate font-mono text-xs text-muted">{{ item.sourceMarkdownPath }}</p>
            </div>
          </div>
        </section>
        ...
        <aside class="rounded-lg border border-default bg-default p-4 xl:sticky xl:top-24 xl:self-start">
          <div class="mb-4 flex items-start justify-between gap-3">
            <div>
              <p class="text-xs font-medium uppercase tracking-wide text-muted">批次操作</p>
              <h2 class="mt-1 text-base font-semibold text-highlighted">{{ statusMeta(batch.status).label }}</h2>
            </div>
          ...
          <div class="space-y-3 rounded-lg border border-default bg-elevated/35 p-3 text-sm">
            <div class="flex items-center justify-between gap-3">
              ...
              <span class="font-mono text-xs text-default">{{ items.length }}</span>
            </div>
            <div class="flex items-center justify-between gap-3">
              <span class="text-muted">阻塞</span>
              <span class="font-mono text-xs text-default">{{ summary?.blocking ? 'yes' : 'no' }}</span>
            </div>
            <div class="flex items-center justify-between gap-3">
              <span class="text-muted">警告</span>
              <span class="font-mono text-xs text-default">{{ summary?.warnings ?? 0 }}</span>
            </div>
          </div>

          <USeparator class="my-4" />

          <div v-if="batch.status === 'completed'" class="rounded-lg border border-warning/30 bg-warning/5 p-3">
            <div v-if="!rollbackConfirm" class="space-y-3">
              <div>
                <p class="text-sm font-medium text-highlighted">回滚这次导入</p>
                <p class="mt-1 text-xs leading-5 text-muted">创建的文档会被删除，更新和归档会尽量恢复到导入前快照。</p>
              </div>
              <UButton label="回滚导入" icon="i-tabler-history" color="warning" variant="soft" block @click="() => { rollbackConfirm = true }" />
            </div>
            <div v-else class="space-y-3">
              <p class="text-sm text-highlighted">确定回滚这个导入批次？</p>
              <div class="flex justify-end gap-2">
                <UButton label="取消" color="neutral" variant="ghost" size="sm" @click="() => { rollbackConfirm = false }" />
                <UButton label="确认回滚" icon="i-tabler-history" color="warning" size="sm" :loading="rollbackBusy" @click="rollback" />
              </div>
            </div>
          </div>
          ...
          <UAlert
            ...
            title="当前状态不可回滚"
            :description="batch.status === 'rolled_back' ? '这个批次已经回滚。' : '只有已完成的导入批次可以回滚。'"
          />
        </aside>
      </div>
    </template>
