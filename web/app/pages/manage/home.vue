...
const collectionItems = computed(() => collections.value.map(item => ({ label: item.title, value: item.slug })))
const loading = computed(() => !mounted.value || collectionsPending.value || homePending.value)
      ...
      <div class="grid gap-5 xl:grid-cols-[minmax(0,1fr)_380px]">
        <section class="min-w-0 rounded-lg border border-default bg-default">
          <div class="flex flex-wrap items-center justify-between gap-3 border-b border-default bg-elevated/35 px-4 py-3">
            <div>
              <h2 class="text-sm font-semibold text-highlighted">快速入口配置</h2>
              <p class="mt-0.5 text-xs text-muted">配置首页入口卡片的标题、路径和展示状态。</p>
            </div>
            <UButton icon="i-tabler-plus" label="添加入口" color="neutral" variant="soft" @click="addQuickLink" />
          </div>
              ...
              >
                    ...
                    >
                      ...
                      <template #content>
                        <ManageIconPicker
                          ...
                          compact
                          @update:model-value="chooseQuickLinkIcon(link, $event)"
                        />
                      </template>
                    </UPopover>
                    <div class="min-w-0">
                      <p class="truncate text-sm font-semibold text-highlighted">{{ link.title || `入口 ${index + 1}` }}</p>
                      <p class="mt-0.5 truncate text-xs text-muted">{{ quickLinkTarget(link) }}</p>
                    </div>
                  </div>
                  <UDropdownMenu :items="quickLinkActionItems(link, index)" :ui="{ content: 'w-36' }">
                    <UButton
                ...
                <div class="grid gap-3">
                  <div class="grid gap-3 md:grid-cols-2">
                    <UFormField label="标题">
                      <UInput v-model="link.title" placeholder="快速开始" class="w-full" />
                    </UFormField>
                  ...
                  <UFormField label="说明">
                    <UTextarea v-model="link.description" :rows="2" class="w-full" />
                  </UFormField>

                  <div class="grid gap-3 md:grid-cols-[minmax(0,1fr)_auto] md:items-end">
                    <UFormField label="自定义链接">
                      <UInput v-model="link.to" placeholder="留空时使用文档集路径" class="w-full" />
                    </UFormField>
                    <UCheckbox v-model="link.enabled" label="公开展示" class="pb-2" />
                  </div>
        ...
        <aside class="space-y-5 xl:sticky xl:top-20 xl:self-start">
          <section class="rounded-lg border border-default bg-default">
            <div class="border-b border-default bg-elevated/35 px-4 py-3">
              ...
              <p class="mt-0.5 text-xs text-muted">选择首页展示的文档集，右侧顺序按选择顺序生成。</p>
            </div>
            <div class="grid gap-2 p-4">
              <button
                ...
                :class="featuredCollections.includes(collection.slug) ? 'border-primary/40 bg-primary/10 text-primary' : 'border-default bg-elevated/35 text-default hover:border-primary/30'"
                @click="toggleFeatured(collection.slug)"
              >
                <span class="flex min-w-0 items-center gap-3">
                  <span class="grid size-8 shrink-0 place-items-center rounded-md bg-default text-primary ring-1 ring-default">
                    <UIcon :name="collection.icon || 'i-tabler-stack-2'" class="size-4" />
                  </span>
                  <span class="min-w-0">
                    <span class="block truncate text-sm font-medium">{{ collection.title }}</span>
                    <span class="block truncate font-mono text-xs opacity-70">/{{ collection.slug }}</span>
                  </span>
                </span>
                <UIcon :name="featuredCollections.includes(collection.slug) ? 'i-tabler-circle-check' : 'i-tabler-circle'" class="size-5 shrink-0" />
              </button>
            </div>
          </section>
          ...
          <section class="rounded-lg border border-default bg-default p-4">
            ...
            <div v-if="featuredCollections.length" class="mt-3 space-y-2">
              <div v-for="(slug, index) in featuredCollections" :key="slug" class="flex items-center justify-between gap-3 rounded-md bg-elevated px-3 py-2 text-sm">
                <span class="truncate">{{ index + 1 }}. {{ collectionTitle(slug) }}</span>
