...
const { ensure, tree, collection } = useCollectionTree(
...
if (!collection.value)
  throw createError({
    statusCode: 404,
    statusMessage: "文档集不存在",
    fatal: true,
  });
const startNodes = computed(() => tree.value.slice(0, 4));
const showStartNodes = computed(
  () => tree.value.length > startNodes.value.length,
);
useSeoMeta({
  ...
  <div class="min-w-0 space-y-10 overflow-x-hidden">
    <header class="border-b border-default pb-8">
      ...
      <div class="min-w-0 max-w-[72ch]">
        <p
          class="mb-3 inline-flex items-center gap-2 rounded-full bg-primary/10 px-3 py-1 text-xs font-medium text-primary"
        >
          <UIcon :name="collection?.icon || 'i-tabler-book-2'" class="size-4" />
          文档集
        </p>
        <h1
          class="font-display text-balance text-3xl font-bold leading-tight tracking-tight text-highlighted sm:text-[2.25rem]"
        >
          {{ collection?.title }}
        </h1>
        <p
          v-if="collection?.description"
          class="mt-4 max-w-2xl break-words text-base leading-7 text-muted"
        >
          {{ collection.description }}
        </p>

        <div
          class="mt-5 flex min-w-0 flex-wrap items-center gap-x-4 gap-y-2 text-sm text-muted"
        >
          <span class="inline-flex min-w-0 items-center gap-2">
            <span
              class="relative size-7 shrink-0 overflow-hidden rounded-md bg-elevated ring-1 ring-default"
            >
              <img
              ...
              <span
                v-else
                class="grid size-full place-items-center text-primary"
              >
                <UIcon
            ...
            <span class="font-medium text-default">{{
              rootSectionCountLabel(tree.length)
            }}</span>
          </span>
    ...
    <section v-if="showStartNodes" class="space-y-4">
      ...
      <div class="grid gap-3 sm:grid-cols-2">
        ...
        >
          <span
            class="mt-0.5 grid size-8 shrink-0 place-items-center rounded-lg bg-primary/10 text-primary"
          >
            <UIcon name="i-tabler-file-text" class="size-4" />
          </span>
          <span class="min-w-0">
            <span
              class="line-clamp-2 font-medium text-highlighted transition group-hover:text-primary"
              >{{ n.title }}</span
            >
            <span
              v-if="n.excerpt"
            ...
            >
            <span
              v-else-if="n.children?.length"
              class="mt-1 block text-sm text-muted"
              >{{ n.children.length }} 个子章节</span
            >
          </span>
        </NuxtLink>
      </div>
    ...
    <section class="space-y-4">
        ...
        >
          <div class="flex min-w-0 items-start justify-between gap-3">
            <div class="min-w-0">
              ...
              <p
                v-if="n.excerpt"
                class="mt-1 line-clamp-2 text-sm leading-6 text-muted"
              >
                {{ n.excerpt }}
              </p>
            </div>
          ...
          <p v-if="n.children?.length" class="mt-3 text-xs text-muted">
            {{ n.children.length }} 个子章节
          </p>
        </NuxtLink>
      </div>
      ...
      <div
        v-else
        class="rounded-2xl border border-dashed border-default px-4 py-10 text-center sm:p-10"
      >
