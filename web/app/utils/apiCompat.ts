...
export interface DocsApiCallOptions {
...
function isLegacyEnvelope(value: unknown): value is LegacyEnvelope<unknown> {
  return typeof value === 'object' && value !== null && 'code' in value &&
    typeof value.code === 'string' && 'data' in value
}
...
function problemParams(value: unknown) {
  ...
  return Object.fromEntries(Object.entries(value).filter(([, item]) => {
    ...
    return Array.isArray(item) && item.every(part => ['string', 'number', 'boolean'].includes(typeof part))
  }))
}

function safeTraceId(value: unknown, response: Response) {
  const candidate = response.headers.get('x-trace-id') ?? (typeof value === 'string' ? value : undefined)
  return candidate && /^[A-Za-z0-9._:-]{1,128}$/.test(candidate) ? candidate : `legacy-http-${response.status}`
}

export async function decodeDocsLegacyFailure(response: Response, limit: number): Promise<ApiFailure> {
  ...
  try {
    ...
    return { kind: 'protocol', code: 'foundation.problem.invalid_body', reauth: 'not-attempted' }
  }
...
export function decodeDocsApiResponse<T>(value: unknown): T {
  if (!isLegacyEnvelope(value)) return value as T
  if (value.code !== 'ok') throw new Error('foundation.response.legacy_envelope_error')
...
export function splitDocsApiUrl(url: string) {
  ...
  return { path: path as `/${string}`, query: queryFromUrl(search) }
}

export function toDocsApiRequest<T>(
  url: string,
  options: DocsApiCallOptions = {},
): { path: `/${string}`, options: RequestOptions<T> } {
  const parsed = splitDocsApiUrl(url)
  const method = String(options.method ?? 'GET').toUpperCase() as HttpMethod
  const headers = options.headers
    ? Object.fromEntries(new Headers(options.headers)) as RequestOptions<T>['headers']
    : undefined
  return {
    path: parsed.path,
    options: {
      ...
      decode: decodeDocsApiResponse<T>,
      decodeFailure: decodeDocsLegacyFailure,
    },
  }
}
