export interface Collection {
  ...
  coverAssetId?: string; coverUrl: string; icon: string; sortOrder: number; docCount: number
}
// CollectionView is the API response shape for collection list/detail endpoints.
export type CollectionView = Collection
export interface DocBase {
  id: string; collectionId: string; versionId: string; parentId: string; slug: string
  title: string; excerpt: string; status: string
  seoTitle?: string; seoDescription?: string
  locale: string; translationKey: string; sortOrder: number
}
...
export interface CollectionTree { collection: Collection; tree: DocTreeNode[] }
export interface CollectionManageTree {
  collection: Collection
  tree: DocDetail[]
}
export interface PublicDocDetail { doc: DocDetail }
export interface DocDetailResponse { doc: DocDetail }
export interface CollectionList { items: Collection[] }
export interface MeView { sub: string; authenticated: boolean; isOwner: boolean }
...
export interface CollectionVersionsResponse { items: CollectionVersion[] }
