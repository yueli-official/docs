    ...
    groups.push([
      {
        ...
        onSelect: () =>
          emit("move", {
            dragId: node.id,
            targetId: props.nodes[index - 1]!.id,
            position: "before",
          }),
      },
      {
...
<template>
  <ul class="space-y-0.5">
    <li v-for="n in nodes" :key="n.id" class="relative">
      ...
      <!-- node row -->
      <div
        class="group flex items-center gap-2 rounded-md px-2 py-1.5 hover:bg-elevated"
        :class="{
          'cursor-grab': canMove,
          'opacity-50': dragState.node?.id === n.id,
          'ring-2 ring-inset ring-primary':
            dropInfo?.nodeId === n.id && dropInfo.position === 'into',
        }"
        :draggable="canMove"
      ...
      >
        ...
        <span
          v-if="
            confirmingId !== n.id &&
            (canEdit || canCreate || canMove || canDelete)
          "
        ...
        >
          ...
          />
          <UDropdownMenu
            v-if="canMove || canDelete"
            :items="nodeActionItems(n, nodes.indexOf(n))"
            :ui="{ content: 'w-40' }"
          >
            <UButton
      ...
      <div
        ...
        class="ml-3 mt-0.5 border-l border-default pl-2"
      >
        <DocTreeAdmin
          :nodes="n.children"
