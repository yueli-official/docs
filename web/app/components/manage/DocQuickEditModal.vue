...
const { doc, docs } = defineProps<{
  doc?: QuickEditDoc;
  docs: readonly QuickEditDoc[];
}>();
  ...
  while (changed) {
    ...
    for (const item of docs) {
      if (
        ...
        (item.parentId !== id && !descendants.has(item.parentId))
      )
        continue;
...
const parentItems = computed(() => {
  ...
  return [
    ...
    ...docs
      .filter(
        (item) =>
          item.collectionId === doc.collectionId &&
          item.versionId === doc.versionId &&
...
function reset() {
  ...
  state.slug = doc.slug;
  state.status = schema.shape.status.safeParse(doc.status).success
    ? (doc.status as Schema["status"])
    : "draft";
  state.parentId = doc.parentId || "";
  submitError.value = "";
}
  ...
  >
    <template #body>
      ...
      >
        ...
        <div
          class="grid items-start gap-5 sm:grid-cols-[minmax(0,2fr)_minmax(10rem,1fr)]"
        >
          <UFormField
            name="slug"
            label="Slug"
            description="修改后公开路径及子文档路径会随之变化。"
            required
    ...
    <template #footer>
      <div
        class="flex w-full flex-col-reverse justify-end gap-2 p-4 sm:flex-row sm:items-center"
      >
        <UButton
          label="打开完整编辑器"
